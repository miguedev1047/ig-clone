...
export function SavedPosts() {
  ...
  if (savedPosts.length === MIN_COUNT)
    return (
      <div>
        <h2 className='font-semibold text-2xl'>There are not saved posts</h2>
      </div>
    )

  const SAVED_POSTS = savedPosts?.map(({ post }) => (
    <li
  ...
  return <ul className='grid grid-cols-3 gap-1'>{SAVED_POSTS}</ul>
}
